...
class CalibrationVerification(Node):
    def __init__(self):
        ...
        # UR3e
        self.ur_ip = "192.168.1.10"  # UR3e IP 地址
        ...
        self.start_time = self.get_clock().now()  # 记录启动时间
        self.initial_wait_duration = 2  # 修改等待时间为2秒
        self.position_history = [] # 记录位置历史用于检测稳定性
        self.stability_duration = 10.0 # 需要稳定10秒
        self.stability_threshold = 0.01 # 允许的最大偏差 1cm
                    ...
                    if M["m00"] != 0:
                        ...
                        if depth_val > 0:
                            ...
                            try:
                                ...
                                self.position_history.append((current_time_sec, point_base.point))
                                
                                # 移除过期的历史记录
                                self.position_history = [p for p in self.position_history if current_time_sec - p[0] <= self.stability_duration]
                                ...
                                if len(self.position_history) > 1:
                                    ...
                                    xs = [p[1].x for p in self.position_history]
                                    ys = [p[1].y for p in self.position_history]
                                    zs = [p[1].z for p in self.position_history]
                                    max_dev = max(max(xs)-min(xs), max(ys)-min(ys), max(zs)-min(zs))
                                    
                                    if duration >= self.stability_duration and max_dev < self.stability_threshold:
                                        is_stable = True
                                ...
                                if is_stable:
                                    ...
                                    self.get_logger().info("=" * 60)
                                    self.get_logger().info(f"🎯 检测到木块且位置稳定!")
                                    ...
                                    user_input = input("位置已稳定10秒。是否移动到目标位置? (y/n): ")
